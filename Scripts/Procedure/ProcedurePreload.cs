...
namespace FirstBattle
{
    ...
    {
        private string[] m_DataTableNames = Array.Empty<string>();
        ...
        private void PreloadResources()
        {
            ...
            //LoadConfig("DataTableNames");

            LoadDataTableNames("DataTableNames");

            if (m_DataTableNames != null && m_DataTableNames.Length > 0)
            {
                // Preload data tables
                foreach (string dataTableName in m_DataTableNames)
                {
        ...
        private void LoadDataTableNames(string configName)
        {
            string assetPath = AssetUtility.GetConfigAsset(configName, false);
            string data = File.ReadAllText(assetPath);
            if (!string.IsNullOrEmpty(data))
            {
                m_DataTableNames = data.Split(",");
            }
            else
            {
